...
fun ServerEditDialog(
    ...
    when (LocalUiMode.current) {
        UiMode.Miuix -> {
            OverlayDialog(
                show = show,
                title = if (server == null) strings.addServer else "Edit Server",
                ...
                content = {
                    Column(Modifier.fillMaxWidth()) {
                        ...
                        TextField(
                            ...
                            label = strings.serverHost,
                            modifier = Modifier.fillMaxWidth().padding(bottom = 8.dp)
                        )
                        TextField(
                            value = port,
                            onValueChange = { port = it },
                            label = strings.serverPort,
                            modifier = Modifier.fillMaxWidth().padding(bottom = 8.dp)
                        )
                        TextField(
                            value = password,
                            onValueChange = { password = it },
                            label = strings.serverPassword,
        ...
        UiMode.Material -> {
            androidx.compose.material3.AlertDialog(
                onDismissRequest = onDismiss,
                title = { Text(if (server == null) strings.addServer else "Edit Server") },
                text = {
                    Column(Modifier.fillMaxWidth()) {
                        OutlinedTextField(
                            value = name,
