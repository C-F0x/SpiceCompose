...
class DampedDragAnimation(
    ...
    fun updateValue(value: Float) {
        val targetV = value.coerceIn(valueRange)
        animationScope.launch {
            launch { valueAnimation.animateTo(targetV, valueAnimationSpec) { updateVelocity() } }
        }
    }
    ...
    fun animateToValue(value: Float) {
        animationScope.launch {
            mutatorMutex.mutate {
                press()
                val targetV = value.coerceIn(valueRange)
                launch { valueAnimation.animateTo(targetV, valueAnimationSpec) }
                if (velocity != 0f) {
